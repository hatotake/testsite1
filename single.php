...
   <script>
	$(window).on('load scroll resize', function() {
		...
		var bottom = $('footer').offset().top - 200;
		var content = $('.d').height() + 0;
...
<div class=d>
	<div class="article-snsbox">
		...
		?>
		<ul class="clearfix">
			<li><?php if (function_exists('scc_get_share_twitter')) : ?><div class="arrow_box"><span><?php echo scc_get_share_twitter() ?></span></div><?php endif; ?>
				<a href="http://twitter.com/share?text=<?php echo $title_encode ?>&url=<?php echo $url_encode ?>&tw_p=tweetbutton&via=<?php the_author_meta('twitter'); ?>&related=<?php the_author_meta('twitter'); ?>" class="naifix-sns-button">
					<div class="sns-button-image sbi-twitter"><i class="fa fa-twitter" id="fat" aria-hidden="true"></i><span style="margin-right: 10px;">Twitter</span></div>
				</a></li>

			<li class="face">
				<div id="fb-root"></div>
				<script async defer crossorigin="anonymous" src="https://connect.facebook.net/ja_JP/sdk.js#xfbml=1&version=v10.0&appId=806845033563385&autoLogAppEvents=1" nonce="vY8XJ7Pd"></script>

				<div class="fb-like" data-href="https://engineer-labo.com/" data-width="" data-layout="box_count" data-action="like" data-size="small" data-share="false"></div>
				<?php if (function_exists('scc_get_share_facebook')) : ?><?php endif; ?>
				<a href="http://www.facebook.com/sharer.php?src=bm&u=<?php echo $url_encode; ?>&t=<?php echo $title_encode; ?>" class="naifix-sns-button">
					<div class="sns-button-image sbi-facebook"><i class="fac fab fa-facebook-f fa-2x" aria-hidden="true"></i>
						<p>シェア</p>
					</div>
				</a>
			</li>

			<li><?php if (function_exists('scc_get_share_hatebu')) : ?><div class="arrow_box"><span><?php echo scc_get_share_hatebu() ?></span></div><?php endif; ?>
				<a href="http://b.hatena.ne.jp/add?mode=confirm&url=<?php echo $url_encode ?>" class="naifix-sns-button">
					<div class="sns-button-image sbi-hatenab"><span>Ｂ!</span>Bookmark</div>
				</a></li>

			<li><?php if (function_exists('scc_get_share_pocket')) : ?><div class="arrow_box"><span><?php echo scc_get_share_pocket() ?></span></div><?php endif; ?>
				<a href="http://getpocket.com/edit?url=<?php echo $url_encode ?>&title=<?php echo $title_encode; ?>" class="naifix-sns-button">
					<div class="sns-button-image sbi-pocket"><i class="fa fa-get-pocket" aria-hidden="true"></i><span style="margin-right: 10px;">Pocket</span></div>
				</a></li>

			<li>
				<p class="arrow_box koudoku">購読</p><a href='https://feedly.com/i/subscription/feed%2Fhttps%3A%2F%2Fengineer-labo.com%2Ffeed%2F' target='blank'><img id='feedlyFollow' src='http://s3.feedly.com/img/follows/feedly-follow-rectangle-flat-big_2x.png' alt='follow us in feedly' width='90' height='56'></a>
			</li>

			<li id="line">
				<div class="line-it-button" data-lang="ja" data-type="share-a" data-ver="3" data-url="https://engineer-labo.com/" data-color="default" data-size="small" data-count="false" style="display: none;"></div>
				...
				<div id="post-<?php the_ID(); ?>" <?php post_class('st-post'); ?>>
					...
					<?php if (!is_attachment()) : ?>
						<div id="breadcrumb" <?php if ($show_post_info) : ?> class="st-post-data-breadcrumb" <?php endif; ?>>
							...
							?>
								<h1 class="entry-title st-css-no"><?php if ($st_is_ex) : st_the_title();
																	else : the_title();
																	endif;    // タイトル 
																	?></h1>
							<?php endif; ?>
					...
					<?php if (have_posts()) : ?>
						<?php while (have_posts()) :
							...
							?>
								...
								<h1 class="entry-title"><?php if ($st_is_ex) : st_the_title();
														else : the_title();
														endif;    // タイトル 
														?></h1>
							...
							?>
								<div style="display:none;"><?php get_template_part('itiran-date-singular');    // 投稿日 
															?></div>
							<?php endif; ?>
							...
							<div class="sns-page">
								...
								<?php get_template_part('sns');    // ソーシャルボタン読み込み 
								?>
								<?php if ($st_is_ex_af) : get_template_part('st-author');
								endif;    // 記事を書いた人 
								?>
